...
local function IsScpCameraBlocked(ply)
...
local function LerpCam(speed, from, to)
	return LerpFT(speed * cz_lerp_camera:GetFloat(), from, to)
end
...
function HGAddView(ply, origin, angles, velLen)
	if ply:Alive() then
		...
		lerped_ang = LerpCam(0.2, lerped_ang, ang * (inSight and 1 or 1) * math.max(org.recoilmul or 1, 0.1))
		...
		walkLerped = LerpCam(0.1, walkLerped, ply:InVehicle() and 0 or vellenlerp * 100)
		
		local walk = math.Clamp(walkLerped / 100, 0, 1)
		
		walkTime = walkTime + walk * FrameTime() * 2 * game.GetTimeScale() * (ply:OnGround() and 1 or 0)
		
		velocityAddVel = LerpCam(0.9, velocityAddVel * 0.9, -vel * 0.1)
	
		velocityAdd = LerpCam(0.1, velocityAdd, velocityAddVel)
...
CalcView = function(ply, origin, angles, fov, znear, zfar)
	...
	lply = IsValid(lply) and lply or ply
	//do return end
	...
	if not ply:Alive() and not follow then
		if lply:GetNWInt("viewmode",0) == 1 then
			local spectTarget = lply:GetNWEntity("spect", NULL)
	...
	local vehiclebase = ply:GetVehicle()
...
function hg.cam_things(ply, view, angles)
	...
	offsetView = LerpCam(0.001, offsetView, angleZero)
	...
	angles[3] = angles[3] - (lean_lerp or 0) * 7
end
...
hook.Add( "HG.InputMouseApply", "FreezeTurning", function( tbl )
	if not altlook then
		LookY = LerpCam(0.1, LookY, 0)
		LookY = math.abs(LookY) > 0.01 and LookY or 0
		LookX = LerpCam(0.1, LookX, 0)
...
local function renderscene(pos, angle, fov)
	...
	if IsScpCameraBlocked(lply) then return end
...
zfar = LerpCam(0.005, zfar, not util.IsSkyboxVisibleFromPoint(pos) and 55000 or mapswithfog[map])
