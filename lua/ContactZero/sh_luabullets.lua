...
function ENTITY:FireLuaBullets(tInfo)
	if not istable(tInfo) then return end
	if isnumber(tInfo.Spread) then
		tInfo.Spread = Vector(tInfo.Spread, tInfo.Spread, 0)
	elseif not tInfo.Spread then
	...
	local seed = owner.GetMD5Seed and owner:GetMD5Seed() or 0
	seed = tonumber(seed) or 0
	local iSeed = seed % 0x100 - 1
