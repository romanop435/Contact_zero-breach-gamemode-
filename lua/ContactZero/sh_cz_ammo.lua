...
local function onstopped_silk(self, last_unsure_penetration_pos, reason, trace)
	if(SERVER)then
		if(!trace or !trace.HitSky)then
			...
			if(trace)then
				projectile:Hit(trace.Entity, trace.HitPos - normal * 10, trace.PhysicsBone)
			end
		end
	end
end
...
if CLIENT then
    ...
    concommand.Add( "cz_ammomenu", function( ply, cmd, args )
        AmmoMenu(ply)
    end )
	...
	hook.Add("radialOptions", "hg-ammomenu", function()
		...
		if not organism.otrub and table.Count(LocalPlayer():GetAmmo()) > 0 then
			hg.radialOptions[#hg.radialOptions + 1] = {function() RunConsoleCommand("cz_ammomenu") end, "Drop Ammo"}
		end
	end)
end
