...
local function createButton(k,ent,size,Pan,mainpan)
    ...
    timer.Simple(0.1,function()
        but.Entity:SetSkin(ent.SKIN or 0)
        ...
        function VPan:DoClick()
            ...
            mainpan.RPanel.Entity:SetSkin(ent.SKIN)
        ...
        BPan:SetText( LocalPlayer():PS_HasItem(ent.ID) and "SOLD" or "Buy: "..ent.PRICE..(ent.ISDONATE and " DZP" or " ZP") )
        BPan:SetFont( "HomigradFontMedium" )
        function BPan:DoClick()
            if self.InWait then return end
            if LocalPlayer():PS_HasItem(ent.ID) then 
                self:SetText( LocalPlayer():PS_HasItem(ent.ID) and "SOLD" or "Buy: "..ent.PRICE..(ent.ISDONATE and " DZP" or " ZP") ) 
            return end
            self:SetText( "Wait..." )
            self.InWait = true 
        ...
        SPan:SetSize( col:GetWide()/1.4, col:GetTall() )
            ...
            Pan:SetRowHeight( size * 0.805 )
                ...
                for k, ent in pairs(PLUGIN.Items) do
                    if not ent.ISDONATE then continue end
