...
if CLIENT then
	...
	function SWEP:DrawWorldModel2()
        ...
        if (IsValid(owner)) and (ent == owner or hg.KeyDown(owner,IN_USE) or (owner:GetNetVar("lastFake",0) > CurTime())) then
            ...
            local Time = math.Truncate(timing,2)
			local Event = self.AnimsSounds and self.AnimsSounds[WorldModel:GetSequenceName(WorldModel:GetSequence())] or false
			self.oldEvent = self.oldEvent or nil
			if Event and Event[ Time ] then
				if self.oldEvent != Event[ Time ] then
					Event[ Time ]( self )
					self.oldEvent = Event[ Time ]
				end
			else
				self.oldEvent = nil
			end

            self.sprintanim = qerp(0.01 * FrameTime() / engine.TickInterval(),self.sprintanim or 0,(owner.IsSprinting and owner:IsSprinting()) and 1 or 0)
...
function SWEP:Camera(eyePos, eyeAng, view, vellen)
	...
	self.walkinglerp = Lerp(hg.lerpFrameTime2(0.1),self.walkinglerp or 0,owner:InVehicle() and 0 or hg.GetCurrentCharacter(owner):GetVelocity():LengthSqr())
