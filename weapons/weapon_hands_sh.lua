...
function SWEP:SetHandPos(noset)
	...
	self.rhandik = ((self:GetFists() and not self:GetBlocking())) or (IsValid(ent) and twohands)
	self.lhandik = (((self:GetFists() and not self:GetBlocking())) and hg.CanUseLeftHand(ply)) or IsValid(ent)
	...
	if self:GetFists() then
		...
		if self.rhandik then
			for _, bone in ipairs(bones) do
				...
				ply_bonematrix:SetTranslation(bonepos + posadd * -0.2 - ang:Right() * 2)
		...
		if self.lhandik then
			for _, bone in ipairs(bones) do
				...
				ply_bonematrix:SetTranslation(bonepos + posadd * -0.7 + ang:Right() * 2)
				ply_bonematrix:SetAngles(boneang)
	...
	if SERVER then
		...
		elseif IsValid(tr.Entity) and tr.Entity:IsPlayer() then
			...
			if Dist < self.ReachDistance then
				...
				if self:GetOwner().organism.superfighter then
...
function SWEP:PrimaryAttack(forcespecial)
	...
	self:SetNextPrimaryFire(CurTime() + .35 * math.Clamp((180 - self:GetOwner().organism.stamina[1]) / 90,1,2) + (special_attack and 0.5 or 0))
	self:SetNextSecondaryFire(CurTime() + .35 + (special_attack and 0.5 or 0))
