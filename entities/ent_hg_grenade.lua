...
if SERVER then
	...
	local function createSpoon(self)
		local entasd = ents.Create("prop_physics")
    ...
    function ENT:PlaySndExplosion(snd, server, chan, vol, pitch, entity, tripleaffirmative)
        ...
        if tripleaffirmative then
            EmitSound(snd, self:GetPos() - Vector(0, 0, 10), (entity or self:EntIndex()) + 1, chan, vol, 140, nil, pitch + rand - 5)
            EmitSound(snd, self:GetPos() + Vector(0, 0, 10), (entity or self:EntIndex()) + 2, chan, vol * 0.9, 140, nil, pitch + rand + 5)
