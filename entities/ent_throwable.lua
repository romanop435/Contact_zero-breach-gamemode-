...
if CLIENT then
	...
	function ENT:Think()
		...
		self:SetCollisionGroup(speed < 640000 and COLLISION_GROUP_WEAPON or COLLISION_GROUP_NONE)
	end

	function ENT:PhysicsCollide(data, phys)
		if data.Speed < 800 then return end
		...
		if (data.HitEntity:IsPlayer() or data.HitEntity:IsRagdoll()) and ((self.DamageType or DMG_SLASH) == DMG_SLASH) then
		...
		dmginfo:SetDamage((self.damage or 20) * (data.Speed / self.MaxSpeed))
