...
if SERVER then
	...
	function ENT:Think()
		if self:GetPhysicsObject():GetVelocity():Length() < 50 or self:WaterLevel() > 0 and not self.Exploded then
			local ent = ents.Create("weapon_cz_molotov_tpik")
	...
	function ENT:Detonate()
		...
		timer.Simple(.01, function()
			...
			for i = 0, 25 do
				...
				if Tr.Hit then
					...
					CreateVFireBall(30, 12, Tr.HitPos + vector_up * 4, self.Velocity/2 - vector_up * 10 + VectorRand() * 150,self.owner or self)
				else
		...
		--local Flame = ents.Create("ent_cz_firesmall")
